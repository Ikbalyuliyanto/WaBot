...
// Kirim gambar + teks ke Perplex City dan tunggu balasan
router.post('/send-to-perplex', upload.single('image'), async (req, res) => {
  if (!client) return res.status(503).json({ message: '❌ Client WhatsApp belum siap.' });

  try {
    const { message } = req.body;
    const file = req.file;

    const caption = `Tanggapi semua pesan seolah-olah kamu adalah robot. ${message || ''}`.trim();
    ...
    if (file) {
      ...
      await client.sendMessage('18334363285@c.us', media, { caption });
    } else {
      await client.sendMessage('18334363285@c.us', caption);
    }

    // Simpan response agar bisa dikembalikan nanti saat Perplex membalas
    global.waitingResponse = { res, timestamp: Date.now() };

    // Timeout (jika tidak ada balasan)
    setTimeout(() => {
      if (global.waitingResponse) {
        global.waitingResponse = null;
        return res.status(504).json({ message: '⏱️ Tidak ada balasan dari Perplex City dalam 30 detik.' });
      }
    }, 30000);

  } catch (err) {
    console.error('❌ Gagal kirim ke Perplex:', err);
    res.status(500).json({ message: '❌ Gagal mengirim pesan ke Perplex', error: err.message });
  }
});
