...
function initHeaderComponent() {
  ...
  function getUserNamaSafe() {
    ...
    try {
      ...
      return payload?.nama || payload?.email?.split("@")[0] || "User";
    } catch {
      return "User";
    }
  }
  ...
  function renderUserDropdown() {
    ...
    if (!nameEl || !subEl || !itemsEl) return;
  ...
  async function loadCartDropdown() {
    ...
    // ✅ Jika tidak ada token, langsung tampilkan pesan login
    if (!token) {
      itemsEl.innerHTML = `
        <div class="cart-dropdown__empty">
    ...
    try {
      ...
      // ✅ Handle 401 SETELAH fetch — token expired/invalid
      if (res.status === 401) {
        localStorage.removeItem('token');
        sessionStorage.removeItem('token');
        itemsEl.innerHTML = `
      ...
      itemsEl.innerHTML = items.map(item => {
        ...
        const imgSrc = gambar.startsWith('http') ? gambar : `${API_BASE}${gambar}`;

        return `
          <div class="cart-item">
            <div class="cart-item__image">
              ${gambar
                ? `<img src="${imgSrc}" alt="${escapeHtml(nama)}" loading="lazy" width="60" height="60">`
                : '<span style="font-size:24px;">📦</span>'}
            </div>
            <div class="cart-item__info">
              <div class="cart-item__name">${escapeHtml(nama)}</div>
              ${variantText ? `<div class="cart-item__variant">${escapeHtml(variantText)}</div>` : ''}
      ...
      if (totalEl) totalEl.textContent = `Rp ${Number(totalPrice).toLocaleString('id-ID')}`;
...
(function () {
  ...
  window.includeHeaderFooter = function (callback) {
    Promise.all([
      ...
      .catch(err => {
        // ✅ Silent fail untuk menghindari console error di Lighthouse
...
async function loadKategoriDropdown() {
  ...
  try {
    ...
    // ✅ Silent fail
    if (body) body.innerHTML = '<div style="padding: 20px; text-align: center; color: #ff4444; font-size: 14px;">Gagal memuat kategori</div>';
  }
}
